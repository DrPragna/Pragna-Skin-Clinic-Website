...
/**
 * FOOTER - Compact Brand Moment
 * 
 * Design Philosophy:
 * - Compact, refined layout
 * - Clean, organized links
 * - Premium feel with reduced footprint
 * - Subtle animated ticker/marquee
 */
...
const footerLinks = {
  ...
  social: [
    ...
    { name: 'WhatsApp', href: 'https://wa.me/918886531111', icon: 'whatsapp' },
  ],
};
    ...
    <footer ref={containerRef} className="relative text-cream overflow-hidden">
      ...
      <div className="absolute inset-0 z-0">
        <div
          ...
          style={{ backgroundImage: 'url(/images/clinic-interior.webp)' }}
        />
              ...
              <div className="flex items-center justify-center lg:justify-start gap-2.5">
                {footerLinks.social.map((social) => (
                  <a
                    ...
                    className="w-9 h-9 rounded-full border border-cream/20 flex items-center justify-center hover:bg-cream hover:text-black transition-all duration-300"
              ...
              <ul className="space-y-2">
                {footerLinks.main.map((link) => (
                  <li key={link.name}>
                    <Link
                      href={link.href}
                      className="text-cream/70 text-sm hover:text-rose-gold transition-colors duration-300"
                    >
                      {link.name}
                    </Link>
                  </li>
